...
const ResumeDownloader = ({ variant = "button", className = "" }) => {
	...
	const downloadPDF = async () => {
		...
		try {
			...
			const response = await fetch("/CV_Rene_Kristofer_Pohlak.pdf", {
				method: "HEAD",
			});
			...
			setError("Resume PDF is currently unavailable. Please try again later.");
		} finally {
			setIsLoading(false);
		}
	};
	...
	return (
		<AssetErrorBoundary
			fallbackMessage="Resume download is currently unavailable."
			showRetry={true}
		>
			{error ? (
				<div
					className={`${currentStyle} ${className} opacity-75 cursor-not-allowed`}
				...
				<button
					onClick={downloadPDF}
					disabled={isLoading}
					className={`${currentStyle} ${className} ${isLoading ? "opacity-75 cursor-wait" : ""}`}
